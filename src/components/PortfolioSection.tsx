      ...
      <div className="flex items-center justify-between p-6 border-b border-gray-200">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Manage your stock investments</p>
          </div>
        </div>
        <button
          onClick={() => setIsAddingStock(true)}
          className="inline-flex items-center space-x-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white px-4 py-2 rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md"
        >
      ...
      <div className="p-6">
          ...
          <div className="bg-gradient-to-r from-green-50 to-green-100 rounded-xl p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-green-900">{getTotalStocks()}</p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3">
              {getSectorDistribution().map((item) => (
                <div key={item.sector} className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className={`w-3 h-3 rounded-full ${sectorColors[item.sector] || 'bg-gray-500'}`}></div>
                    <span className="text-sm font-medium text-gray-700">{item.sector}</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <div className="w-24 bg-gray-200 rounded-full h-2">
                      <div 
                        className={`h-2 rounded-full ${sectorColors[item.sector] || 'bg-gray-500'}`}
        ...
        {isAddingStock && (
          <motion.div
            ...
            className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl p-6 mb-6 border border-blue-200"
          >
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {popularStocks.map((stock) => (
                  <button
                    ...
                    className="p-3 bg-white border border-gray-200 rounded-lg text-left hover:border-blue-300 hover:bg-blue-50 transition-all duration-200"
                  >
                    <p className="font-medium text-gray-900 text-sm">{stock.symbol}</p>
                    <p className="text-xs text-gray-500 truncate">{stock.name}</p>
                    <p className="text-xs text-blue-600 font-medium">₹{stock.price}</p>
                  </button>
                ))}
              </div>
            </div>
        ...
        <div className="space-y-4">
          {stocks.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-4">Add some stocks to get started with portfolio tracking</p>
              <button
                onClick={() => setIsAddingStock(true)}
              ...
              >
                <div className="flex-1">
                  <div className="flex items-center space-x-4">
                    <div className={`w-3 h-3 rounded-full ${sectorColors[stock.sector] || 'bg-gray-500'}`}></div>
                    <div>
